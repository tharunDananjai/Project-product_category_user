...
const registerWithOtp =  async (req, res) => {
  ...
  try {
    ...
    newUser.save().then((result) => {
      sendOtpVerificationEmail(result.email, result.otp,res);
    });
...
//==================================> OTP Verification
const registerOtpVerification = async (req,res)=> {
    let {id,otp} = req.body;
    let userExist = await User.findOne({_id:id})
    if(userExist){
      if(userExist.otp == otp){
        let userUpdate = await User.updateOne({ verified: true });  
          res.send("OTP Verified Succesfully!!!");
      }
      else{
...
const verifyEmail = async  (req, res, next) => {
    ...
    try {
      ...
      if(!decoded) return res.status(403).json({ status: 403, message: "The user email id does not exists" })
        ...
        let userUpdate = await User.update({ verified: true });
...
module.exports = {registerWithOtp, registerOtpVerification, verifyEmail, register, login, editUser, deleteUser,forgotPassword }